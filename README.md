...
В начале скрипта **vm_conf.sh** проводятся подготовительные действия для начала работы с ZFS - устанавливаются необходимые пакеты и модули.
Далее в скрипте выполняется набор команд, согласно методичке. Выполнение скрипта занимает около 10 минут.\
В этом же файле (README.md) приводятся команды с выводом и комментариями.
### 1. Определение алгоритма с наилучшим сжатием ###
